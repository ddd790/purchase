...
public class OrderDao {
	...
	public int addOrder(Order order) {
		String sql = "INSERT INTO orders (PROJECT_CODE,TITLE,COMPANY_NAME,CUSTOMER_COMPANY_CODE,CUSTOMER_USER_CODE,"
				...
				+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		return jdbcTemplate.update(sql, order.getPROJECT_CODE(), order.getTITLE(), order.getCOMPANY_NAME(), order.getCUSTOMER_COMPANY_CODE(), order.getCUSTOMER_USER_CODE(),
				order.getCONTRACT_CODE(), order.getCONTRACT_DATE(), order.getCONTRACT_USER_CODE(), order.getORDERED_FLG(), order.getCONFIRMER_USER_CODE(), order.getREMARKS(),
				order.getREGISTER_DATE(), order.getREGISTER_USER_NAME(), order.getUPDATE_DATE(), order.getUPDATE_USER_NAME(), order.getDELETE_FLG());
	}
